...
image1 = cv2.imread("depth_files/artroom/im0.png")
image2 = cv2.imread("depth_files/artroom/im1.png")
...
k_l = np.array([[1733.74 ,0, 792.27],[0, 1733.74, 541.89],[0,0,1]])
k_r = np.array([[1733.74 ,0, 792.27],[0, 1733.74, 541.89],[0,0,1]])
#image shapes 
...
def compute_essential_matrix(F,k_l,k_r):
    ...
    print("The Essential Matrix for Artroom Dataset is:")
    print(E)
    return E
...
def sift_matcher(img1,img2,image1_color,image2_color):
    ...
    for match1,match2 in matches:
        if match1.distance < 0.25*match2.distance:
            good.append([match1])
    ...
    cv2.imwrite("artroom/SIFTMatchingPoints.jpg",sift_matches)
...
print("The Fundamental Matrix for artroom dataset is :")
print(F)
...
print("Rotation matrix for artroom dataset is:")
print(R_final)
print("Translation matrix for artroom dataset is:")
print(T_final)
...
cv2.imwrite("artroom/LeftEpilineBefore.jpg",left_img1)
image1_copy = image1.copy()
image2_copy = image2.copy()
...
cv2.imwrite("artroom/RightEpilineBefore.jpg",right_img1)
epi_img = np.concatenate((left_img1,right_img1),axis=1)
cv2.imwrite("artroom/InitialEpiline.jpg",epi_img)
...
cv2.imwrite("artroom/left_epiline.jpg",left_img1_r)
image1_copy = image1.copy()
image2_copy = image2.copy()
...
cv2.imwrite("artroom/right_epiline.jpg",right_img1)
epi_img_r = np.concatenate((left_img1_r,right_img1_r),axis=1)
cv2.imwrite("artroom/epiline.jpg",epi_img_r)
...
cv2.imwrite("artroom/DisparityMap.jpg",disparity_map)
cv2.imwrite("artroom/DisparityHeatMap.jpg",disparity_heatmap)
